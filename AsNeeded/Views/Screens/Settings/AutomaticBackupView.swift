...
struct AutomaticBackupView: View {
    ...
    var body: some View {
        ...
        .alert("Automatic Backups Need Reconfiguration", isPresented: $viewModel.showingBackupReconfigAlert) {
            Button("OK", role: .cancel) {}
        } message: {
    ...
    private var backupContentSection: some View {
        VStack(alignment: .leading, spacing: contentSpacing) {
            ...
            VStack(spacing: 12) {
                Toggle(isOn: $viewModel.automaticBackupIncludeSettings) {
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Include App Settings")
                            .font(.customFont(fontFamily, style: .body))
                        Text("Include app preferences (fonts, haptics, privacy settings)")
                            .font(.customFont(fontFamily, style: .caption))
                            .foregroundStyle(.secondary)
                    }
                }
