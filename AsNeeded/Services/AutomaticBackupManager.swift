...
final class AutomaticBackupManager: ObservableObject {
	...
	private func performBackup() async {
		logger.info("Performing automatic backup (redactNames: \(self.redactMedicationNames), redactNotes: \(self.redactNotes))")
		...
		do {
			...
			let exportData = try await DataStore.shared.exportDataAsJSON(
				redactNames: self.redactMedicationNames,
				redactNotes: self.redactNotes
			)
			...
			switch error {
			case .insufficientStorage(let needed, _):
