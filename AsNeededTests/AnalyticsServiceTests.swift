...
struct AnalyticsServiceTests {
	...
	@MainActor
	func testTrackDataImportFailure() {
		let analytics = AnalyticsService.shared
		let initialSummary = analytics.getAnalyticsSummary()
	...
	@MainActor
	func testAverageDailyLaunchesNoFirstLaunch() {
		let summary = AnalyticsSummary(
			firstLaunchDate: nil,
			launchCount: 10,
			lastLaunchDate: Date(),
	...
	@MainActor
	func testTrackFeaturePerformance() {
		let analytics = AnalyticsService.shared
	...
	@MainActor
	func testGetSummaryPerformance() {
		let analytics = AnalyticsService.shared
	...
	func testTopFeaturesSorting() {
		...
		for i in 0..<features.count-1 {
			#expect(features[i].count >= features[i+1].count)
		}
	}
	
	@Test("Top features limited to 5")
	@MainActor
	func testTopFeaturesLimit() {
		let analytics = AnalyticsService.shared
