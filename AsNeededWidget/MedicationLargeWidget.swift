...
struct LargeWidgetProvider: TimelineProvider {
	...
	func placeholder(in context: Context) -> FullMedicationListEntry {
	...
	func getSnapshot(in context: Context, completion: @escaping (FullMedicationListEntry) -> Void) {
		Task { @MainActor in
	...
	func getTimeline(in context: Context, completion: @escaping (Timeline<FullMedicationListEntry>) -> Void) {
		Task { @MainActor in
			let entry = await createEntry()
